...
**It's worth noting that we can't have propensity scores that are exactly 0 or exactly 1, since that will give us Inf for our logit transformation. So, the entries from the regression tree that match this criteria are added/subtracted a small dx so that they don't go to infinity.
```{r}
...
Now, we'll proceed with the full matching, using the MatchIt package. This part is the most time intensive, since it must find the optimal matching such that total discrepancy between matched comparisons is minimized, so it can't take a greedy approach. (Hansen, 2004) 
```{r}
